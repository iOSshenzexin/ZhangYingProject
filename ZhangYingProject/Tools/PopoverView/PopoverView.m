...
+(PopoverView *)sharedPopview{
    static PopoverView *vc = nil;
    if (!vc) {
        vc = [[PopoverView alloc] init];
    }
    return vc;
}
...
//    UILabel *textView = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, textSize.width, textSize.height)];
    UILabel *textView = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, ScreenW-40, textSize.height)];
...
#pragma mark xiugai
    DealController *vc = [DealController sharedDealController];
    NSIndexPath *indexPath = [NSIndexPath indexPathForRow:0 inSection:0];
    DealTopCustomCell *cell = [vc.dealTableView cellForRowAtIndexPath:indexPath];
    UIView *topView = cell.backView;
...
- (void)tapped:(UITapGestureRecognizer *)tap {
    ...
    self.point = point;
    if ([self.delegate respondsToSelector:@selector(popoverViewdidSetupPopview:)]) {
        [self.contentView removeFromSuperview];
        [self.delegate popoverViewdidSetupPopview:self];
        //[self dismiss];
